...
import { fetchDevices, pairDevice, unpairDevice, runDeviceDiagnostic, isLiveResponse } from '../../data/api';

export default function TechnicianDevices() {
  const { id } = useParams();
  
  const { data: devices, isLoading, refetch } = useApi(() => fetchDevices(id || '1'), {
    dependencies: [id],
    cacheKey: `devices-${id || '1'}`
  });

  const isLive = isLiveResponse(devices);
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold text-navy">Biomarker Devices</h2>
            <p className="text-sm text-slate-muted">Hardware assigned for high-fidelity physiological monitoring</p>
          </div>
          {isLive && (
        ...
        <button 
          ...
          className="bg-navy text-white px-6 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-black transition-all shadow-lg active:scale-95 duration-200"
        >
          <Zap className="w-4 h-4 text-amber" /> Pair New Device
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {deviceList.length > 0 ? (
          deviceList.map((device: any, idx: number) => (
            <div key={device.id || idx} className="bg-card rounded-3xl border border-light-blue overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col justify-between">
              <div className="p-6">
                ...
                <div className="mb-6">
                  <p className="text-[10px] text-slate-muted uppercase font-bold tracking-widest mb-1">{device.serial || device.id || 'DEVICE'}</p>
                  <h3 className="text-lg font-bold text-navy">{device.name || 'Unnamed Device'}</h3>
                  <p className="text-sm text-slate-muted">{device.category || device.type || 'Clinical Asset'}</p>
                </div>

                <div className="space-y-3 pt-6 border-t border-light-blue">
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-muted flex items-center gap-2">
                      <Battery className={`w-4 h-4 ${parseInt(device.battery || '100') < 20 ? 'text-coral' : 'text-sage'}`} />
                      Battery Life
                    </span>
                    <span className="font-bold text-navy">{device.battery || '95%'}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-muted">Last Sync</span>
                    <span className="font-bold text-navy">{device.last_sync_human || device.lastSync || 'Recently'}</span>
                  </div>
                  <div className="flex justify-between items-center text-sm">
                    <span className="text-slate-muted">Assigned On</span>
                    <span className="font-bold text-navy">{device.assigned_date || device.assigned || '—'}</span>
                  </div>
                </div>
              </div>
              
              <div className="p-4 bg-background border-t border-light-blue flex gap-3 shrink-0">
                 <button 
                   onClick={() => handleUnpair(device.id, device.name)}
                   disabled={isSubmitting}
                   className="flex-1 py-2.5 bg-card border border-light-blue text-[10px] font-bold uppercase tracking-widest text-coral hover:bg-red-50 hover:border-red-200 rounded-lg transition-all disabled:opacity-40"
                 >
                   Unpair
                 </button>
                 <button 
                   ...
                   className="flex-1 py-2.5 bg-card border border-light-blue text-[10px] font-bold uppercase tracking-widest text-navy hover:bg-background rounded-lg transition-all flex items-center justify-center gap-1 disabled:opacity-40"
                 >
          ...
          <div className="col-span-full py-20 text-center bg-card rounded-3xl border border-light-blue">
            <Activity className="w-12 h-12 text-slate-muted mx-auto mb-4 opacity-20" />
            <p className="text-slate-muted font-bold uppercase tracking-widest text-xs">No devices assigned to this patient.</p>
          </div>
        )}
      </div>
      ...
      {showPairModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-navy/40 backdrop-blur-sm">
          <form onSubmit={handlePair} className="bg-card rounded-[2rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-bold text-navy">Pair New Device</h3>
              <button type="button" onClick={() => setShowPairModal(false)} className="text-slate-muted hover:text-navy">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="block text-xs font-bold text-slate-muted uppercase tracking-wider mb-2">Device Name</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-muted uppercase tracking-wider mb-2">Device Type</label>
                <select
                  value={newDeviceType}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-muted uppercase tracking-wider mb-2">Serial Number</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-background border border-light-blue rounded-xl p-3 text-sm focus:ring-2 focus:ring-amber outline-none font-mono"
                />
              </div>
            </div>
